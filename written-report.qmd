...
As you'll notice, the dataset splits the base stats of each pokémon into the individual stats, but we only need to know about the base_total variable, Variables like is_legendary, may prove to be extremely important in our regression model, while with others, like name and Japanese name, we can remove them since they are simply unique identifiers.
