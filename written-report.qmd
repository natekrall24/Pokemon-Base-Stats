...
We first randomly split our data of 800 different pokemon into 75% training and 25% testing data. Note that we removed one pokemon from the original 801 pokemon, Minior, from the dataset, since it has 2 different forms and has an uninterpretable capture rate. We noticed that while reading the .csv file, R automatically translated the \*capture_rate\* variable to characters because Minior's capture rate was listed as: "30 (Meteorite)255 (Core)". We decided to exclude this observation from the model because of its uninterpretable characteristics, and after, we casted \*capture_rate\* an integer instead of a character.
...
3.  We remove all against\_\* variables, since the predictive power of these variables are already represented by their types, so we reduce redundancy and collinearity by doing so.

4.  We create 1-hot encoding dummy variables for type1 and type2 so they can be used in our model.

5.  We make capture rate a numeric variable, as it is not a numeric variable in the original data set.

6.  We convert is_legendary to a factor, as it is more useful as a categorical variable in our model.
...
```{r}
pokemon_fit_model <- extract_fit_parsnip(pokemon_initfit)
glance(pokemon_fit_model)$r.squared
```
...
```{r}
#to get RSE and RSQ
collect_metrics(pokemon_initfit)
```
