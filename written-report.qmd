...
```{r}
pokemon_rec <- recipe(base_total ~ ., data = pokemon_train) |> #make name's role ID 
...
```{r}
pokemon_spec <- linear_reg() |>
  set_engine("lm")

pokemon_wflow <- workflow() |>
...
```{r}
pokemon_spec2 <- linear_reg() |>
  set_engine("lm")

pokemon_wflow2 <- workflow() |>
...
```{r}
pokemon_secondfit <- pokemon_wflow2 |>
  fit(data = pokemon_train)

tidy(pokemon_secondfit) |>
  kable(digits = 3)
```
...
```{r}
glance(pokemon_initfit) |> 
  select(AIC, BIC, adj.r.squared)

glance(pokemon_secondfit) |> 
  select(AIC, BIC, adj.r.squared)
```
...
pokemon_wflowV <- workflow() |>
	add_model(pokemon_spec) |>
	add_recipe(pokemon_rec)

pokemon_initfitV <- pokemon_wflowV |>
	fit_resamples(folds)
collect_metrics(pokemon_initfitV)
```
...
```{r}
set.seed(123)
folds2 <- vfold_cv(pokemon_train, v = 15)

pokemon_wflowV2 <- workflow() |>
	add_model(pokemon_spec2) |>
	add_recipe(pokemon_rec_less)

pokemon_initfitV2 <- pokemon_wflowV2 |>
	fit_resamples(folds)
collect_metrics(pokemon_initfitV2)
```
...
pokemon_fitless <- extract_fit_parsnip(pokemon_secondfit)
...
Linearity condition - satisfied because there is no clear patterns in the residuals vs predictor variables or a fanning effect

Constant Variance - The vertical spread of the residuals is not constant across the plot

Normality - The distribution of the residuals is approximately unimodal and symmetric, so the normality condition is satisfied. The sample size is sufficiently large \>\> 30 to relax this condition if it was not satisfied.

Independence - The independence condition is satisfied as we established earlier there is little variation in base total over generations (which is the same as over time). Additionally, Pokemon are created to be inherently balanced so there can be no bias in the data collection as the data of individual Pokemon do not change over time.

**Initial results of our model**

```{r}
sqrt(mean((pokemon_aug$.resid)^2))
```

We calculated our RMSE for the final model to be 72.81 base total points, meaning the average residual length is 72.81 base total points.

```{r}
pokemon_fit_model123 <- extract_fit_parsnip(pokemon_fit_final)
glance(pokemon_fit_model123)$r.squared
```

We calculated our R\^2 value for the final model to be 0.6284, meaning 62.84% of the variability in base_total is explain by the predictor variables in our final model.
